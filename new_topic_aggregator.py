...
def generate_new_topics(learning_objects):
    client = MongoClient('mongodb://mongodb:27017/onion', ssl_cert_reqs=ssl.CERT_NONE)
            ...
            for learning_object in released_learning_objects:
                if (learning_object_description == learning_object.get('description')):
                    ...
                    if key in topic_to_cuid_dict:
                        topic_to_cuid_dict[key].append({ 'cuid': learning_object.get('cuid'), 'version': learning_object.get('version'), 'name': learning_object.get('name'), 'author': author })
                    else:
                        topic_to_cuid_dict[key] = [{ 'cuid': learning_object.get('cuid'), 'version': learning_object.get('version'), 'name': learning_object.get('name'), 'author': author }]
    ...
    for index, topic in enumerate(topic_to_cuid_dict):
        for learning_object in topic_to_cuid_dict.get(topic):
            for given_learning_object in learning_objects:
                if given_learning_object.get('cuid') == learning_object.get('cuid'):
                    topic_name = 'new_topic_' + str(index)
